...
run_case composer clean 0 1
run_case composer finding 1 1
run_case composer recover 0 3
run_case composer exhaust 70 3
run_case composer config 2 1
run_case composer mismatch 2 1
run_case composer false-green 2 1
run_case composer malformed 2 1

set +e
PEANUT_AUDIT_MAX_ATTEMPTS=0 "$WRAPPER" composer >/dev/null 2>&1
status=$?
set -e
[ "$status" -eq 2 ] || fail "invalid retry configuration did not fail before scanning"

echo "PEANUT LICENSE SERVER COMPOSER AUDIT TRANSPORT CONTRACT PASSED"
